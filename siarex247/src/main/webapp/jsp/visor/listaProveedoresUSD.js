...
var tablaDetalleUSD = null;

	$(document).ready(function() {
		try {
			tablaDetalleUSD = $('#tablaDetalleUSD').DataTable( {
				paging      : true,
				retrieve: true,
				pageLength  : 10,
				lengthChange: false,
...
//				dom: "<'row mx-0'<'col-md-12'QB><'col-md-6'l><'col-md-6'f>>" + "<'table-responsive scrollbar'tr>" + "<'row g-0 align-items-center justify-content-center justify-content-sm-between'<'col-auto mb-2 mb-sm-0 px-0'i><'col-auto px-0'p>>",
				...
				buttons: [
					{ 	extend: "collection",
				    	...
				    	buttons: [
				    		{ extend: "excel", text:'Exportar Excel',  title: 'Lista Proveedores Mas Populares USD', exportOptions: {modifier: {selected: null, page: 'all'}}, filename: 'detalleProveedores'} 
				        ], 
			      	}
				],
				language : {
				...
				columnDefs: [
					{	targets: 1,
				        render: function (data, type, row) {
				        	rowElement = '<a href="javascript:mostrarOpcion(\'/siarex247/jsp/visor/detalleVisor.jsp?rfcProveedor='+row.rfc+'&tipoMoneda=USD&claveProveedor='+row.claveProveedor+' \');">' + row.rfc + '</a>';
				            return rowElement;
				        }
				    }
					/*
				   {
				        ...
				        render: function (data, type, row) {
				            ...
				            rowElement += '<a class="dropdown-item" href="javascript:abreModal(\'ver\', \'' + row.claveProveedor + '\');">'+BTN_VER_MENU+'</a>';
		          ...
		          initComplete: function () {
		        	  ...
		        	  var VISOR_ETQ19 = $('#VISOR_ETQ19');
		        	  VISOR_ETQ19.removeClass('text-end');
		        	  
		        	  var VISOR_ETQ20 = $('#VISOR_ETQ20');
		        	  VISOR_ETQ20.removeClass('text-end');
		        	  
		        	  var VISOR_ETQ21 = $('#VISOR_ETQ21');
		        	  VISOR_ETQ21.removeClass('text-end');
		...
		tablaDetalleUSD.on( 'draw', function () {
	...
	$('#tablaDetalleUSD thead tr:eq(1) th').each( function (i) {
		 var title = $(this).text();
		 if (i == 5){
		 } else {
	...
	$('#tablaDetalleUSD thead tr:eq(1) th').on('keyup', "input",function () {
		filtraDatosUSD($(this).parent().index(), this.value);
	});
		
	function filtraDatosUSD(columna, texto) {
		tablaDetalleUSD
			.column(columna)
	        .search(texto)
	        .draw();

...
var tablaDetalleValidarCP = null;

	$(document).ready(function() {
		try {
			tablaDetalleValidarCP = $('#tablaDetalleValidarCP').DataTable( {
				paging      : true,
				retrieve: true,
				pageLength  : 15,
				lengthChange: false,
...
//				dom: "<'row mx-0'<'col-md-12'QB><'col-md-6'l><'col-md-6'f>>" + "<'table-responsive scrollbar'tr>" + "<'row g-0 align-items-center justify-content-center justify-content-sm-between'<'col-auto mb-2 mb-sm-0 px-0'i><'col-auto px-0'p>>",
				...
				ajax : {
					url: '/siarex247/visor/tablero/detalleClaveProductoCartaPorte.action',
					data : {
						folioEmpresa : function() { return obtenerFolioEmpresa_CP() }
		...
		tablaDetalleValidarCP.on( 'draw', function () {
					...
					$.ajax({
						...
						success  : function(data) {
							if($.isEmptyObject(data)) {
								...
								if ( data.estatusOrden == 'A12'){
									consultarInformacion_CP();
									$('#myModalValidarCP').modal('show');
								}else{
									Swal.fire({
			  			                title: MSG_ERROR_OPERACION_MENU,
	...
	function consultarInformacion_CP(){
		try{
			var cadFolios = getSelectionsEmpresa();
			var arrFolios = cadFolios.split(";");
			var folioEmpresa = arrFolios[0]; 
			$('#folioEmpresa_ValidarCP').val(folioEmpresa);
			
			$.ajax({
				url  : '/siarex247/visor/tablero/consultarClaveProductoCartaPorte.action',
				type : 'POST', 
				data : {
				...
				success  : function(data) {
					if($.isEmptyObject(data)) {
						...
						if (data.bandActivo == 'A'){
							$('#valExito_ValidarCP').show();
						}else {
							$('#valError_ValidarCP').show();
						}
						$('#razonSocial_ValidarCP').val(data.razonSocial);
						$('#rfc_ValidarCP').val(data.rfc);
						$('#ordenCompra_ValidarCP').val(data.folioEmpresa);
						$('#usoCFDI_ValidarCP').val(data.usoCFDI);
						$('#descripcionUso_ValidarCP').val(data.desUsoCFDI);
						
						$('#tablaDetalleValidarCP').DataTable().ajax.reload(null,false);
						
					}
				},
				error : function(xhr, ajaxOptions, thrownError) {
					alert('consultarInformacion_CP()_'+thrownError);
				}
			});
		}catch(e){
			alert('consultarInformacion_CP()_'+e);
		}
	}
	
	
	function guardarUsoCartaPorte(tipoAccion){
		try{
			
			var usoCFDI = $('#usoCFDI_ValidarCP').val();
			var folioEmpresa = $('#folioEmpresa_ValidarCP').val();
			
	 		$.ajax({
				url  : '/siarex247/visor/tablero/modificaOrdenClaveProductoCartaPorte.action',
				type : 'POST', 
				data : {
				...
				beforeSend: function( xhr ) {
        			$('#btnAprobarCartaPorte').prop('disabled', true);
        			$('#btnRechazarCartaPorte').prop('disabled', true);
        		},
        		complete: function(jqXHR, textStatus){
        			$('#btnAprobarCartaPorte').prop('disabled', false);
        			$('#btnRechazarCartaPorte').prop('disabled', false);
	    		},
				dataType : 'json',
				success  : function(data) {
				...
				error : function(xhr, ajaxOptions, thrownError) {
					alert('guardarUsoCartaPorte()_'+thrownError);
				}
			});			
			
		}catch(e){
			alert('guardarUsoCartaPorte()_'+e);
		}
	 }
	
	function obtenerFolioEmpresa_CP(){
		try{
			return $('#folioEmpresa_ValidarCP').val();
		}catch(e){
			alert("obtenerFolioEmpresa_CP()_"+e);
		}
	}
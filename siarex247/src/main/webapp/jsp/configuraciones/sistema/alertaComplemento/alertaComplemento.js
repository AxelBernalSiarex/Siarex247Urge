	...
	function iniciaTabAlarmaComplemento() {
		
		/* Reset al Formulario */ 
		$("#form-AlarmaComplementos").find('.has-success').removeClass("has-success");
	    $("#form-AlarmaComplementos").find('.has-error').removeClass("has-error");
		$('#form-AlarmaComplementos')[0].reset(); 
		$('#form-AlarmaComplementos').removeClass("was-validated"); 
	...
	function cargaListaProcesar(id) {
		...
		*/
	function consultaAlarmaComplemento(){
		$.ajax({
			url  : '/siarex247/configSistema/alertaComplemento/datosConfProceso.action',
			type : 'POST', 
			dataType : 'json',
			success  : function(data) {
				if($.isEmptyObject(data)) {
					...
					$('#fechaPago_Complemento').val(data.fechaPago);
					$('#txtSubject_Complemento').val(data.subject);
					//$('#txtMsjError_Complemento').val(data.mensajeError);
					$('#txtMsjError_Complemento').summernote('code', data.mensajeError);
					
					$('#txtDestinatario1_Complemento').val(data.destinatario1);
					$('#txtDestinatario2_Complemento').val(data.destinatario2);
					$('#txtDestinatario3_Complemento').val(data.destinatario3);
					$('#txtDestinatario4_Complemento').val(data.destinatario4);
					$('#txtDestinatario5_Complemento').val(data.destinatario5);
					$('#diasProcesar_Complemento').val(data.diasProcesar);
					
					if (data.activar == "S"){
						$('#swtActivarProceso_Complemento')[0].checked = true;
						$('#swtActivarProcesoLbl_Complemento').html("SI"); 
					}else{
						$('#swtActivarProceso_Complemento')[0].checked = false;
			...
			error : function(xhr, ajaxOptions, thrownError) {
				alert('consultaAlarmaComplemento()_'+thrownError);
			}
		});	
    }
	...
	function guardarComplementos(){
		try{
				var activar = $('#swtActivarProceso_Complemento').prop('checked') ? "S": "N";
				var texto = $('#txtMsjError_Complemento').summernote('code');
				
				
			    var formData = new FormData(document.getElementById("form-AlarmaComplementos"));
	            formData.append("activar", activar);
	            formData.append("texto", texto);
	            
	            
	            $.ajax({
	            	url  : '/siarex247/configSistema/alertaComplemento/configuracionProceso.action',
	                dataType: "json",
	                type: "post",
	                data: formData,
	                cache: false,

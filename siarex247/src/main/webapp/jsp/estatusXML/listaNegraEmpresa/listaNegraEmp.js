...
function cargaCmbSupuestos(){
	...
	function obtenerTipoFactura(){
		  var tipoFactura = $('#cmbTipoFactura').val();
	...
	function refrescarEmpresa(){
		...
		if (annio == '' || annio.trim() == '' ||  annio == 0){
			...
			$("#detColumnas").load('/siarex247/jsp/estatusXML/listaNegraEmpresa/listaColumnas.jsp?annioConsulta='+annioConsulta);
		}
	...
	function generarCSV(){
		 try{
				...
				var tipoFactura  = obtenerTipoFactura();
				
			 	  $.ajax({
						url  : '/siarex247/cumplimientoFiscal/listaNegraEmpresa/generarCSV.action',
						data : {
	...
	function buscarRFC(){
		...
		$.ajax({
			url: '/siarex247/cumplimientoFiscal/listaNegraEmpresa/buscarRFC.action',
			type : 'POST', 
			data : {
				rfcListaNegra : rfcListaNegra
			},
	...
	function descargarPDF(){
		try {
			var rfcListaNegra = $('#txtRFC').val();
			 document.getElementById('rfcListaNegra_Buscar').value = rfcListaNegra;

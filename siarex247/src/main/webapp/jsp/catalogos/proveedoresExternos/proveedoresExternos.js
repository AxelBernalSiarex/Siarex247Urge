...
//				dom: "<'row mx-0'<'col-md-12'QB><'col-md-6'l><'col-md-6'f>>" + "<'table-responsive scrollbar'tr>" + "<'row g-0 align-items-center justify-content-center justify-content-sm-between'<'col-auto mb-2 mb-sm-0 px-0'i><'col-auto px-0'p>>",
				...
				buttons: [
					{ 	// text: '<span class="fas fa-plus" data-fa-transform="shrink-3 down-2"></span><span class="d-none d-sm-inline-block ms-1">Nuevo</span>',
			      	...
			      	{ 	extend: "collection",
				    	...
				    	buttons: [
				    		{ extend: "excel", text:'Exportar Excel',  title: 'Catálogo Proveedores Externos', exportOptions: {modifier: {selected: null, page: 'all'}}, filename: 'CatalogoExternos'},
				    		{ extend: "pdf", text:'Exportar PDF', orientation: "landscape", pageSize: "LEGAL", title: "Catálogo Proveedores Externos", filename: 'CatalogoExternos', exportOptions: { modifier: { page: "all", }, }, }, 
				        ], 
			      	}
				],
				language : {
				...
				ajax : {
					url: '/siarex247/catalogos/proveedoresExt/listaDetalle.action',
					type: 'POST'
				},
				aoColumns : [
				...
				columnDefs: [
					{	targets: 4,
				        render: function (data, type, row) {
				        	rowElement = '<a href="javascript:abreModal(\'ver\', \'' + row.claveRegistro + '\');">' + row.rfc + '</a>';
				            return rowElement;
				        }
				    },
				   {
	...
	function iniciaFormCatalogo() {
		...
		$('#idReceptor').val(''); // Selecciona primer valor del combo
		$('#idReceptor').trigger('change'); //Refresca el combo
	...
	function buscaCatalogo(claveRegistro, accion){
		$.ajax({
			url  : '/siarex247/catalogos/proveedoresExt/consultarProveedor.action',
			type : 'POST', 
			data : {
			...
			success  : function(data) {
				if($.isEmptyObject(data)) {
					...
					cargaProveedoresExt(data.idReceptor);
	...
	function eliminaCatalogo(claveRegistro, razonSocial){
		try{
			...
			Swal.fire({
				  ...
				  text: LABEL_PROVEEDORES_EXTERNOS + ' : '+razonSocial,
				  showDenyButton: true,
				  ...
				  if (result.isConfirmed) {
					  $.ajax({
				           url:  '/siarex247/catalogos/proveedoresExt/eliminaProveedores.action',
				           type: 'POST',
				            dataType : 'json',
				            data : {
				            	claveRegistro : claveRegistro
	...
	function guardarCatalogo(){
		try{
			   ...
			   if (codigoPostal == ''){
	            ...
	            var formData = new FormData(document.getElementById("form-Catalogo"));
	            $.ajax({
	            	url: '/siarex247/catalogos/proveedoresExt/altaProveedor.action',
	                dataType: "json",
	                type: "post",
	                data: formData,
	                cache: false,
	...
	function actualizarCatalogo(){
		try{
			   ...
			   if (codigoPostal == ''){
	            ...
	            var formData = new FormData(document.getElementById("form-Catalogo"));
	            $.ajax({
	            	url: '/siarex247/catalogos/proveedoresExt/modificaProveedor.action',
	                dataType: "json",
	                type: "post",
	                data: formData,
	                cache: false,

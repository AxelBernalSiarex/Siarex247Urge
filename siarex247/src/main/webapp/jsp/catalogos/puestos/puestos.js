...
//				dom: "<'row mx-0'<'col-md-12'QB><'col-md-6'l><'col-md-6'f>>" + "<'table-responsive scrollbar'tr>" + "<'row g-0 align-items-center justify-content-center justify-content-sm-between'<'col-auto mb-2 mb-sm-0 px-0'i><'col-auto px-0'p>>",
				...
				buttons: [
					{ 	// text: '<span class="fas fa-plus" data-fa-transform="shrink-3 down-2"></span><span class="d-none d-sm-inline-block ms-1">Nuevo</span>',
			      	...
			      	{ 	extend: "collection",
				    	...
				    	buttons: [
				    		{ extend: "excel", text:'Exportar Excel',  title: 'Catálogo de Puestos', exportOptions: {modifier: {selected: null, page: 'all'}}, filename: 'CatalogoPuestos'},
				    		{ extend: "pdf", text:'Exportar PDF', orientation: "landscape", pageSize: "LEGAL", title: "Catálogo de Puestos", filename: 'CatalogoPuestos', exportOptions: { modifier: { page: "all", }, }, }, 
				        ], 
			      	}
				],
				language : {
				...
				ajax : {
					url: '/siarex247/catalogos/puestos/listaPuestos.action',
					type: 'POST'
				},
				aoColumns : [
					{ mData: null, "sClass": "alinearCentro"},
					{ mData: "idPuesto", "sClass": "alinearCentro"},
					{ mData: "nombreCorto", "sClass": "alinearCentro"},
					{ mData: "descripcion"}
				...
				columnDefs: [
					{	targets: 2,
				        render: function (data, type, row) {
				        	rowElement = '<a href="javascript:abreModal(\'ver\', \'' + row.idPuesto + '\');">' + row.nombreCorto + '</a>';
				            return rowElement;
				        }
				    },
				   {
				        ...
				        render: function (data, type, row) {
				            ...
				            rowElement += '<a class="dropdown-item" href="javascript:abreModal(\'ver\', \'' + row.idPuesto + '\');">'+BTN_VER_MENU+'</a>';
				            rowElement += '<a class="dropdown-item" href="javascript:abreModal(\'editar\', \'' + row.idPuesto + '\');">'+BTN_EDITAR_MENU+'</a>';
				            rowElement += '<div class="dropdown-divider"></div>';
				            rowElement += '<a class="dropdown-item text-danger" href="javascript:eliminaCatalogo(\'' + row.idPuesto + '\', \'' + row.descripcion + '\');">'+BTN_ELIMINAR_MENU+'</a>';
				            rowElement += '</div>';
				            rowElement += '</div>';
				                return rowElement;
				          }
	...
	function buscaCatalogo(idPuesto, accion){
		$.ajax({
			url  : '/siarex247/catalogos/puestos/consultaPuestos.action',
			type : 'POST', 
			data : {
				idPuesto : idPuesto
			},
			...
			success  : function(data) {
				if($.isEmptyObject(data)) {
					...
					$('#idRegistro_Catalogo').val(data.idPuesto);
					$('#txtNombreCorto').val(data.nombreCorto);
	...
	function eliminaCatalogo(idPuesto, descripcion){
		try{
			
			Swal.fire({
				  icon : 'question',
				  title: TITLE_DELETE_CATALOGO,
				  text: LABEL_PUESTOS + ' : '+descripcion,
				  showDenyButton: true,
				  ...
				  if (result.isConfirmed) {
					  $.ajax({
				           url:  '/siarex247/catalogos/puestos/eliminaPuestos.action',
				           type: 'POST',
				            dataType : 'json',
				            data : {
				            	idPuesto : idPuesto
	...
	function guardarCatalogo(){
		try{
			...
			var descripcion = $('#txtDescripcion').val();
	 		...
	 		$.ajax({
				url  : '/siarex247/catalogos/puestos/altaPuestos.action',
				type : 'POST', 
				data : {
	...
	function actualizarCatalogo(){
		try{
			var idPuesto = $('#idRegistro_Catalogo').val();
			var nombreCorto = $('#txtNombreCorto').val();
			var descripcion = $('#txtDescripcion').val();
			
	 		$.ajax({
				url  : '/siarex247/catalogos/puestos/actualizaPuestos.action',
				type : 'POST', 
				data : {
					idPuesto : idPuesto,
					nombreCorto : nombreCorto,

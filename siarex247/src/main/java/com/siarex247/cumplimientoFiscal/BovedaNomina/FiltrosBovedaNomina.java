...
public class FiltrosBovedaNomina {
    ...
    /* ========= TEXT ========= */
    public void addTextFilter(Where w, String column, String value, String operator){
    ...
    /* ========= FECHA ========= */
    public void addDateFilter(Where w, String dateOperator, String d1, String d2,
        ...
        if (!op.isEmpty() && !v1.isEmpty()){
            switch (op) {
                case "eq": w.and("E.FECHA_FACTURA BETWEEN ? AND ?", v1+" 00:00:00", v1+" 23:59:59"); break;
                case "ne": w.and("NOT (E.FECHA_FACTURA BETWEEN ? AND ?)", v1+" 00:00:00", v1+" 23:59:59"); break;
                case "lt": w.and("E.FECHA_FACTURA < ?",  v1+" 00:00:00"); break;
                case "gt": w.and("E.FECHA_FACTURA > ?",  v1+" 23:59:59"); break;
                case "le": w.and("E.FECHA_FACTURA <= ?", v1+" 23:59:59"); break;
                case "ge": w.and("E.FECHA_FACTURA >= ?", v1+" 00:00:00"); break;
                case "bt": if (!v2.isEmpty()) w.and("E.FECHA_FACTURA BETWEEN ? AND ?", v1+" 00:00:00", v2+" 23:59:59"); break;
                default:   w.and("E.FECHA_FACTURA BETWEEN ? AND ?", v1+" 00:00:00", (v2.isEmpty()? v1 : v2)+" 23:59:59");
            }
            return;
        }
        // Legado: fechaInicial/fechaFinal
        String fi = nz(fechaInicialLeg), ff = nz(fechaFinalLeg);
        if (!fi.isEmpty() && !ff.isEmpty()) w.and("E.FECHA_FACTURA BETWEEN ? AND ?", fi+" 00:00:01", ff+" 23:59:59");
    }

    /* ========= NUMÉRICOS ========= */
    public void addNumericFilter(Where w, String column, String operator, String v1, String v2){
    ...
    public void addFolio(Where w, String folioTxt, String folioOperator, String folioV1, String folioV2, boolean castToDecimal){
        boolean folioNum = !nz(folioOperator).isEmpty() && (!nz(folioV1).isEmpty() || !nz(folioV2).isEmpty());
        if (folioNum){
            String col = castToDecimal ? "CAST(E.FOLIO AS DECIMAL(20,6))" : "FOLIO";
            addNumericFilter(w, col, folioOperator, folioV1, folioV2);
        } else if (!nz(folioTxt).isEmpty()){
            w.and("E.FOLIO = ?", folioTxt);
        }
    }

    /* IN por si se usa selección de UUIDs (Excel/ZIP) */
    public void addUuidIn(Where w, List<String> uuids){
    ...
    public void aplicarFiltrosNomina(Where w,
        ...
        addTextFilter(w, "E.UUID",            uuid,  uuidOp);

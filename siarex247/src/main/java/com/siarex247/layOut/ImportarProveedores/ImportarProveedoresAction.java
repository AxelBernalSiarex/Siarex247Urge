...
public class ImportarProveedoresAction extends OrdenesCompraSupport {


	private static final long serialVersionUID = -646647340089096216L;

	


	public String cargarImportar() throws Exception {
		Connection con = null;
		  ...
		  if ("".equals(session.getEsquemaEmpresa())){
			return Action.LOGIN;
			  ...
			  if ("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet".equalsIgnoreCase(UtilsFile.getContentType(fileCarga)) || 
				  ...
				  int claveHistorico = ordenesCompraBean.grabarHistorial(con, session.getEsquemaEmpresa(), fileCarga.getName(), 0, "4",1, 0, 0, getUsuario(request), null);
				  if (claveHistorico == -100) {
					  ...
					  ordenesCompraModel.setClaveRegistro(claveHistorico);
						 ...
						 String dirOrdenes = UtilsPATH.REPOSITORIO_DOCUMENTOS +session.getEsquemaEmpresa() + File.separator + "ARCHIVOS_CARGADOS" + File.separator + fileCarga.getName() ;
		...
		try{
		  ...
		  if ("".equals(session.getEsquemaEmpresa())){
			return Action.LOGIN;
			  ...
			  if (resultado == -100) {
				  ordenesCompraModel.setCodError("001");
				  ordenesCompraModel.setMensajeError("Error al iniciar proceso de importar proveedores, consulte a su administrador.");
				}else {
					
				  ordenesCompraModel.setCodError("000");
				  ordenesCompraModel.setMensajeError("El proceso de importar proveedores ha iniciado satisfactoriamente, revise el estatus una vez que ha terminado.");
				  ...
				  String dirOrdenes = UtilsPATH.REPOSITORIO_DOCUMENTOS + session.getEsquemaEmpresa() + File.separator + "ARCHIVOS_CARGADOS" + File.separator + ordenesCompraForm.getNombreArchivo();
					
				 ImportarProveedoresBean importarProvedores = new ImportarProveedoresBean();
				 importarProvedores.iniciaCarga(dirOrdenes, getUsuario(request), session.getEsquemaEmpresa(), empresasForm.getClaveEmpresa(), getClaveRegistro());
